...
| LRN-11 | P0 | Studying a Learned pack is **Review**. Review works exactly like learning, and additionally shows the quick tag buttons and category controls on each card (see §4). |
...
| ID | Pri | Requirement |
|---|---|---|
| CAT-1 | P0 | Categorising and tagging are available **only for words whose pack is Learned**, in **Review** and in the **Explorer** viewer. For other words the controls are not shown (existing tags are still displayed). The app never prompts the user to categorise. |
...
| TAG-1 | P0 | **Quick buttons** on the card: `Positive`, `Negative`, `Neutral`, `Counter-intuitive`, `Multiple meanings`. One tap applies/removes; changes save immediately. |
...
| EXP-5 | P0 | For words of Learned packs the explorer card shows the quick tag buttons and category controls (CAT-1). |
